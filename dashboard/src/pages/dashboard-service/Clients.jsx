  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setError(err.message || 'Não foi possível carregar a lista de clientes.')
      } finally {
  ...
  return (
    <div className="space-y-4">
      <section className="border border-stone-300 bg-linear-to-r from-stone-900 via-stone-800 to-amber-700 text-white p-5">
        <p className="text-xs uppercase tracking-[0.16em] text-amber-200">CRM do prestador</p>
        <h2 className="text-2xl font-semibold mt-1">Clientes e histórico</h2>
        <p className="text-sm text-stone-200 mt-2 max-w-2xl">
          Visualize recorrência, histórico de agendamentos e status de cada cliente.
        </p>
      </section>

      <section className="border border-stone-200 bg-white p-3 flex items-center justify-between gap-3">
      ...
      {loading ? (
        ...
        <div className="border border-stone-200 bg-white p-5 text-sm text-stone-500">Nenhum cliente encontrado.</div>
      ) : (
            ...
            <article key={client.client_id} className="border border-stone-200 bg-white p-4 space-y-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="text-xs text-stone-600">
                  Último agendamento: <strong>{client.ultimo_agendamento_em || '-'}</strong>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-5 gap-2 text-xs">
                ...
                <p className="border border-green-200 bg-green-50 px-2 py-1 text-green-700">Concluídos: <strong>{client.concluidos}</strong></p>
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-stone-600 uppercase tracking-wide">Histórico</p>
                <div className="space-y-1">
                  {(client.historico || []).slice(0, 8).map((item) => (
                    <div key={item.id} className="border border-stone-200 px-2 py-1.5 text-xs text-stone-600 flex flex-wrap items-center justify-between gap-2">
                      <span>{item.date} {item.time} • {item.service?.name || 'Serviço'}</span>
